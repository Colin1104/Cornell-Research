...
bool target_req_cb(modules::target_req::Request  &req, modules::target_req::Response &res)
{
  ...
  while (true)
  {
    try{
      ...
      listener.waitForTransform("map", "base_footprint", gTime, ros::Duration(2));
  ...
  if (res.recon_type == 4)
  {
    while (true)
    {
      try{
        ros::Time gTime = ros::Time::now();
        listener.waitForTransform("map", "stair_point", gTime, ros::Duration(2));
        listener.lookupTransform("map", "stair_point", gTime, stransform);
        break;
      }
      catch (tf::TransformException &ex) {
        ROS_ERROR("%s",ex.what());
    ...
    target.position.x = stransform.getOrigin().getX();
    target.position.y = stransform.getOrigin().getY();
    
    target.orientation.w = stransform.getRotation().getW();
    target.orientation.x = stransform.getRotation().getX();
    target.orientation.y = stransform.getRotation().getY();
    target.orientation.z = stransform.getRotation().getZ();
