...
if __name__ == '__main__':
    ...
    while (not flag):
    	try:
            now = rospy.Time.now()
            listener.waitForTransform('/openni_rgb_optical_frame','/kinectPB', now, rospy.Duration(20.0))
            print "waited"
    ...
    while not rospy.is_shutdown():
        try:
            now = rospy.Time.now()
            listener.waitForTransform('/kinectPB', '/openni_rgb_optical_frame', now, rospy.Duration(20.0))
            ...
            (errorTrans,errorRot) = listener.lookupTransform('/kinectPB', '/calibrated_rgbd_pos', now)
