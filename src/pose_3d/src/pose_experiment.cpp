...
const bool gazebo = true;
bool stationaryNBV = false;
...
void flag_cb(const std_msgs::Int32ConstPtr& msg)
{
  ...
  if (msg->data == 1)
  {
    cout << "Stationary Request Received" << endl;
    stationaryNBV = true;
  }
  else
  {
    cout << "Mobile Request Received" << endl;
    stationaryNBV = false;
  }
  startAlg = true;
}
...
void gz_cb(const nav_msgs::OccupancyGridConstPtr& grid)
{
  map2d = *grid;
  cout << "Got it: " << map2d.data.size() << endl;
}
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher gazebo_pub = node.advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 15);
  ros::Publisher gz_map_pub;
  ros::Subscriber gz_map_sub;
  ros::Publisher switch_pub;
  
  if (gazebo)
  {
    gz_map_pub = node.advertise<nav_msgs::OccupancyGrid>("/move_base/global_costmap/costmap", 15);
    gz_map_sub = node.subscribe("/test/map", 15, gz_cb);
    switch_pub = node.advertise<std_msgs::Bool>("/cloud_throttle", 15);
  }
  
  //sleep(5);
  
  ros::Subscriber sub = node.subscribe("/octomap_binary", 1, map_cb);
  ...
  if (gazebo)
  {
    ...
    gzNBV.model_name = "kinect";
    gzNBV.pose = pose;
    gzNBV.twist = twist;
    gzNBV.reference_frame = "map";
    
    gazebo_pub.publish<gazebo_msgs::ModelState>(gzNBV);
    
    cout << "Start pose_sim_mapping.launch and let me know when map is ready" << endl;
    int temp;
    cin >> temp;
    
    //ros::service::call<std_srvs::Empty>("/test/octomap_server/reset", resetSrv);
    
    cout << "Creating World Map" << endl;
    
    while (map2d.data.size() == 0 && ros::ok())
    {
      ros::spinOnce();
    }
    ...
    metas.resolution = mapRes;
    metas.origin.position.x = -10;
    metas.origin.position.y = -10;
    metas.origin.position.z = 0.0;
    metas.height = 20 / mapRes;
    metas.width = 20 / mapRes;
    
    world.info = metas;
    
    for (int i = 0; i < metas.height; i++)
    {
      for (int j = 0; j < metas.width; j++)
      {
	world.data.push_back(-1);
      }
    }
    
    cout << "Map 2D: " << map2d.info.height << " x " << map2d.info.width << endl;
    
    for (int i = 0; i < map2d.info.height; i++)
    {
      for (int j = 0; j < map2d.info.width; j++)
      {
	if (map2d.data[i * map2d.info.width + j] >= 50)
	{
	  double x = j * map2d.info.resolution + map2d.info.origin.position.x;
	  double y = i * map2d.info.resolution + map2d.info.origin.position.y;
	  
	  cout << "x: " << x << ", y: " << y << endl;
	  int jNew = round((x - metas.origin.position.x) / mapRes);
	  int iNew = round((y - metas.origin.position.y) / mapRes);
	  
	  world.data[iNew * metas.width + jNew] = 100;
	  
	  int bloat = 2;
	  for (int offX = -bloat; offX <= bloat; offX++)
	  {
	    for (int offY = -bloat; offY <= bloat; offY++)
	    {
	      world.data[(iNew + offY) * metas.width + (jNew + offX)] = 100;
	    }
	  }
	}
      }
    ...
    gz_map_pub.publish(world);
    
    cout << "Resetting Xtion to Start" << endl;
    ...
    clock_t init = clock();
    while (double(clock() - init) / CLOCKS_PER_SEC < 5)
    {
      ros::spinOnce();
    }
  ...
  while (ros::ok())
  {
    ...
    while (((map2d.data.size() == 0 && !gazebo) || !mapReceived) && ros::ok())
    {
      ros::spinOnce();
    }
    ...
    {
      if (tree.isNodeOccupied(*iter) && iter.getZ() < minZ)
      {
		...
		if (unNode == NULL)
		{
		  ...
		  for (int u = -1; u <= 1; u++)
		  {
			...
			if (abs(u) + abs(v) + abs(w) == 1)
			{
			  ...
			  if (cellNode != NULL && !tree.isNodeOccupied(cellNode) && tree.keyToCoord(freeKey).z() > minZ)
  ...
  while (true)
  {
    ...
    tree.expand();
    int treeCount = 0;
    for (OcTree::iterator iter = tree.begin_leafs(searchDepth); iter != tree.end_leafs(); iter++)
    {
    ...
    *   if (iter.getCoordinate().z() > ibxMax.z()) ibxMax.z() = iter.getCoordinate().z();
	...
	//cout << "Populated IG : " << infoChunk[index][0] << endl;
      }
    }
 ...
 if (false)
 {
   ...
   {
     if (tree.search(iter.getCoordinate()) == NULL || tree.isNodeOccupied(tree.search(iter.getCoordinate()))) continue;
 ...
 else
 {
   ...
   cout << "2D Res: " << mapRes << endl;
   cout << "Size: " << mapMeta.height * mapMeta.width << endl;
   
   double rangeX = mapMeta.width;
   double rangeY = mapMeta.height;
   
   //youBot Restrictions
   double minHeight = 0.23;
   double maxHeight = 0.62;
   
   //Unrestricted for Gazebo
   if (gazebo)
   {
     minHeight = 0.05;
     maxHeight = searchBoxMax.z();
   }
   ...
   if (gazebo && stationaryNBV)
   {
     cout << "Stationary NBV Search" << endl;
     try {
       gTime = ros::Time::now();
       listener.waitForTransform("map", "camera_link", gTime, ros::Duration(5.0));
       listener.lookupTransform("map", "camera_link", gTime, stransform);
     }
     catch (tf::TransformException &ex) {
       ROS_ERROR("%s",ex.what());
       ros::Duration(1.0).sleep();
     }
     point3d coords(stransform.getOrigin().x(), stransform.getOrigin().y(), stransform.getOrigin().z());
   ...
   else if (!gazebo && stationaryNBV)
   {
     cout << "Stationary NBV Search" << endl;
     try {
       gTime = ros::Time::now();
       listener.waitForTransform("map", "base_footprint", gTime, ros::Duration(5.0));
     ...
     for (double camHeight = minHeight; camHeight <= maxHeight; camHeight += 0.01)
     {
       float yaw, pitch;
       coords.z() = camHeight;
   ...
   {
     cout << "Mobile NBV Search" << endl;
     for (int x = 0; x < rangeX; x+= 10)
     {
       for (int y = 0; y < rangeY; y+= 10)
       {
	 int index = rangeX * y + x;
	 if (map2d.data[index] == 0)
	 {
	   point3d coords(double(x) * mapRes + minX, double(y) * mapRes + minY, 0);
	   if (coords.x() > searchBoxMax.x() || coords.y() > searchBoxMax.y() || coords.x() < searchBoxMin.x() || coords.y() < searchBoxMin.y())
	   {
	     continue;
	   }
	   for (double camHeight = minHeight; camHeight <= maxHeight; camHeight += 0.13)
	   {
	     float yaw, pitch;
	     coords.z() = camHeight;
	     //if (tree.search(coords) == NULL || tree.isNodeOccupied(tree.search(coords))) continue;
 ...
 //--------------------------------------
    ...
    if (!stationaryNBV && maxIG < 10)
    {
      cout << "Algorithm Complete" << endl;
      break;
    }
    else if (stationaryNBV && maxIG < 10)
    {
      cout << "Mooooving on..." << endl;
      stationaryNBV = false;
      stationaryCount = 0;
      continue;
    }
    ...
    if (gazebo)
    {
      ...
      geometry_msgs::Pose pose;
      pose.position.x = NBV.x();
      pose.position.y = NBV.y();
      pose.position.z = NBV.z();
      ...
      if (stationaryNBV)
      {
	cout << "Publishing Stationary NBV" << endl;
	stationaryCount++;
      }
      else
      {
	cout << "Publishing Regular NBV" << endl;
	stationaryNBV = true;
      }
      ...
      cout << "Moving Sensor" << endl;
      gazebo_pub.publish<gazebo_msgs::ModelState>(gzNBV);
      sleep(1);
      
      cloud_throttle.data = true;
      switch_pub.publish(cloud_throttle);
      cout << "Throttle back on" << endl;
      
      clock_t init = clock();
      while (double(clock() - init) / CLOCKS_PER_SEC < 10)
      {
	ros::spinOnce();
      }
    ...
    else
    {
      ...
      if (stationaryNBV)
      {
	nbv_msg.data = {NBV.x(), NBV.y(), NBV.z() - 0.19, DEG2RAD(maxYaw), maxPitch, 1};
	cout << "Publishing Stationary NBV" << endl;
	stationaryCount++;
      }
      else
      {
	nbv_msg.data = {NBV.x(), NBV.y(), NBV.z() - 0.19, DEG2RAD(maxYaw), maxPitch};
	cout << "Publishing Regular NBV" << endl;
	stationaryNBV = true;
      }
